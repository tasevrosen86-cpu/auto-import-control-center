...
Deno.serve(async (request) => {
  ...
  if (draftId) {
    const { data: existing, error } = await db.from('publication_drafts').select('id').eq('id', draftId).maybeSingle();
    if (error) return response({ error: error.message }, 500);
    if (!existing) return response({ error: 'Черновата не е намерена.' }, 404);
  } else {
    const { data, error } = await db.from('publication_drafts').insert({
      ...
      intake_origin: 'PUBLICATION_URL_IMPORT', extraction_status: 'PROCESSING',
  ...
  const { error: draftError } = await db.from('publication_drafts').update({
    title, status: nextStatus, source_type: sourceType, source_url: sourceUrl,
    source_listing_id: listingId, source_vin: sourceVin, source_price_eur: priceEur,
    intake_origin: 'PUBLICATION_URL_IMPORT', extraction_status: missing.length ? 'COMPLETED_NEEDS_REVIEW' : 'COMPLETED',
  ...
  const { error: fieldsError } = await db.from('publication_draft_fields').upsert(fieldRows, { onConflict: 'draft_id,field_key' });
  ...
  if (extraRows.length) {
    const { error } = await db.from('publication_draft_extras').upsert(extraRows, { onConflict: 'draft_id,extra_key' });
    if (error) return response({ error: error.message }, 500);
  }

  const images = Array.isArray(payload.images) ? payload.images.map(asRecord) : [];
  if (images.length) {
    await db.from('publication_draft_images').delete().eq('draft_id', draftId);
    ...
    if (imageRows.length) {
      const { error } = await db.from('publication_draft_images').insert(imageRows);
      if (error) return response({ error: error.message }, 500);
    }
  }

  const { error: jobError } = await db.from('publication_source_jobs').update({
  ...
  await db.from('publication_draft_action_log').insert({
    draft_id: draftId, action: 'PUBLICATION_URL_IMPORT_INGESTED', actor: 'Publication URL importer',

...
function PublicationDraftEditor({draft,field,missing,selectedImages,price,setPrice,busy,currentPublish,onPrepare,onPublish,onEdit,status}:{draft:Draft;field:(key:string)=>string;missing:string[];selectedImages:number;price:string;setPrice:(value:string)=>void;busy:boolean;currentPublish:PublishJob|undefined;onPrepare:()=>Promise<void>;onPublish:()=>Promise<void>;onEdit:(action:string,payload:Record<string,unknown>)=>Promise<void>;status:(value:string)=>string}){
  ...
  return <div className="border-t border-slate-100 p-3">
    ...
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{Object.entries(labels).filter(([key])=>key!=='vehicle_instructions').map(([key,label])=>field(key)&&<div key={key} className="rounded-md bg-slate-50 px-2.5 py-2"><div className="text-[10px] uppercase text-slate-400">{label}</div><div className="mt-0.5 text-xs font-semibold text-slate-700">{field(key)}</div></div>)}</div>
    {field('vehicle_instructions')&&<div className="mt-3 rounded-md border border-slate-200 bg-white"><div className="border-b border-slate-100 px-3 py-2"><h3 className="text-sm font-bold text-slate-800">Описание Канада</h3><p className="text-[10px] text-slate-500">Извлечено от „Vehicle Description“ в канадската обява. Само за четене — служи да се намери точният модел.</p></div><textarea readOnly value={field('vehicle_instructions')} rows={10} className="w-full resize-y rounded-b-md bg-slate-50 p-3 text-[11px] leading-relaxed text-slate-700 outline-none"/></div>}

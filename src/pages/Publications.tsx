...
export function Publications(){
  ...
  const load=useCallback(async()=>{
    const [jobsResult,draftsResult,fieldsResult,imagesResult]=await Promise.all([
    ...
    const failed=[jobsResult.error,draftsResult.error,fieldsResult.error,imagesResult.error].find(Boolean);
  ...
  const status=(value:string)=>value==='COMPLETED'?'Готово':value==='RUNNING'?'Извличане':value==='FAILED'?'Грешка':'На опашка';
